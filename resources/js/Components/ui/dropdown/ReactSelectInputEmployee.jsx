...
const ReactSelectInputEmployee = ({
    label,
    name,
    employees = [], // Specific to employee data
    selected = null,
    onChange,
    ...
    // Format employees into options with the specified display format
    const formattedOptions = [
        { label: "Select Employee", value: "", original: null }, // Default option
        ...employees.map((emp) => ({
            label: `${emp.first_name} ${emp.last_name} (ID:${emp.employee_id})`,
            value: emp.id,
            original: emp,
        })),
    ];
    ...
    const handleChange = (newOption) => {
        ...
        if (!newOption || newOption.value === "") {
            console.log(
                `[${name}] Deselecting or selecting default, calling onChange with null`
            );
            onChange(null);
    ...
    return (
        <div className="flex flex-col gap-1">
            ...
            <Select
                ...
                isClearable={!!selectedOption && selectedOption.value !== ""}
                onClear={handleClear}
...
export default ReactSelectInputEmployee;
