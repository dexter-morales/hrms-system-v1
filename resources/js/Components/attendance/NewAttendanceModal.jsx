...
const NewAttendanceModal = ({
    ...
    const { data, setData, post, put, processing, errors, reset } = useForm({
        employee_id: editAttendance?.employee_id || initialEmployeeId || "",
        date: editAttendance?.date || initialDate || "",
    ...
    useEffect(() => {
        if (show) {
            console.log("Modal opened, editAttendance:", editAttendance);
            if (editAttendance) {
                setData({
    ...
    }, [show, editAttendance, initialDate, initialEmployeeId, setData]);
    ...
    const handleSubmit = (e) => {
        ...
        const formattedData = {
            ...
            punch_in: data.punch_in
                ? new Date(data.punch_in).toISOString().slice(0, 16)
                : null,
            punch_out: data.punch_out
                ? new Date(data.punch_out).toISOString().slice(0, 16)
                : null,
        };
        ...
        if (editAttendance) {
            put(route("attendances.employee.update", editAttendance.id), {
                data: formattedData,
                preserveScroll: true,
                onSuccess: () => {
            ...
            post(route("attendances.employee.store"), {
                data: formattedData,
                preserveScroll: true,
                onSuccess: () => {
