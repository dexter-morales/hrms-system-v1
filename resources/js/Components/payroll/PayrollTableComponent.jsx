...
const PayrollTableComponent = ({
    ...
    const exportToExcelPerLocationPerDepartment = () => {
        try {
            ...
            Object.entries(groupedData).forEach(([key, totals]) => {
                ...
                if (exportData.length > 0) {
                    ...
                    XLSX.utils.sheet_add_aoa(
                        worksheet,
                        [[getTableName("Per Location Per Department").display]],
                        { origin: "A2" }
                    );
                    ...
                    const sheetName = `${totals.location}_${totals.department}`
                        .replace(/\s/g, "_")
                        .slice(0, 31);
            ...
            if (!hasData) {
                toast.error(
                    "No data to export for Per Location Per Department report."
                );
                return;
            }
            ...
            saveAs(
                blob,
                `${getTableName("Per Location Per Department").filePrefix}.xlsx`
            );
        } catch (error) {
            console.error(
                "Per Location Per Department Excel export error:",
                error
            );
            toast.error(
                "Failed to export Per Location Per Department to Excel. Check console for details."
            );
        }
    };
            ...
            <div className="mb-4 flex flex-wrap gap-4">
                ...
                <div className="flex items-end gap-2">
                    ...
                    <Button
                        className="bg-green-700 hover:!bg-red-900 text-white font-semibold"
                        size="sm"
                        onClick={exportToExcelPerLocationPerDepartment}
                    >
                        <span>
                    ...
                    <Button
                        className="bg-green-700 hover:!bg-red-900 text-white font-semibold"
                        size="sm"
                        onClick={exportToExcelPerLocation}
                    >
                        <span>
                            <FontAwesomeIcon icon={faFileExcel} /> Per Location
                        </span>
