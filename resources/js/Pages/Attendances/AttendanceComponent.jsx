...
const AttendanceComponent = () => {
    ...
    const isPunchedIn =
        ...
        recentActivities[0].timestamp.split(" ")[0] === today &&
            ...
            attendances.map((row) => ({
                ...
                "Total Hours":
                    !row.punch_in || !row.punch_out
                        ? "-"
                        : `${(
                              (new Date(row.punch_out) -
                                  new Date(row.punch_in)) /
                                  (1000 * 60 * 60) -
                              (row.break_hours || 0)
                          ).toFixed(2)} hrs`,
            }))
            ...
            },
            {
                onSuccess: (response) => {
                    if (response.props.flash?.success) {
                        toast.success(
                            "PDF export initiated. Download will start shortly."
                        );
                        // Assuming the server returns a file download response
                        window.location.href =
                            response.props.flash.download_url;
                    } else {
                            ...
                            {payslips?.length > 0 ? (
                                payslips?.map((payslip, index) => (
                                    ...
                                    >
                                        ...
                                        <a
                                            href={`/storage/${payslip.file_path}`}
                                            target="_blank"
