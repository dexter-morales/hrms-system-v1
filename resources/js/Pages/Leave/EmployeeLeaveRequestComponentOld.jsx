...
const EmployeeLeaveRequestComponent = () => {
    ...
    const handleApprove = (id, status, row) => {
        Swal.fire({
            ...
            html: `
                ...
                ${
                    row?.image
                        ? '<p>Supporting Image:</p><img src="/storage/leaves/${row.image}" alt="Supporting Document" class="max-w-xs mt-2" />'
                        : ""
                }
            `,
            icon: "question",
    ...
    const filteredMyLeaves = safeMyLeaves.filter(
        (leave) =>
            (leave.employee.name
                .toLowerCase()
                .includes(filterText.toLowerCase()) ||
                leave.employee.id
                    .toString()
                    .includes(filterText.toLowerCase())) &&
            (leaveTypeFilter === "" || leave.leave_type === leaveTypeFilter) &&
            (statusFilter === "" || leave.status === statusFilter)
    );
    ...
    const columns = [
        ...
        },
        {
            name: "Actions",
            cell: (row) =>
                auth.user.employee.employee_id === row.employee.manager_id &&
                row.status === "Pending" &&
                row.approved_by === null ? (
