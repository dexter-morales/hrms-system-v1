    ...
    public function generate(Request $request)
    {
        ...
        foreach ($employees as $employee) {
            try {
                ...
                // Calculate total_working_days based on total worked hours
                $attendances = EmployeeAttendance::where('employee_id', $employee->id)
                    ...
                    ->map(function ($attendance) use ($employee, $GRACE_PERIOD_MINUTES) {
                        ...
                        if ($punchOut->lte($adjustedPunchIn)) {
                            ...
                            $totalHours = ($workedMinutes / 60) - $breakHours;
                            $attendance->hours_worked = round(min(max($totalHours - ($attendance->overtime_hours ?? 0), 0), 9.6), 2); // Cap at 9.6 hours
                ...
                $totalWorkedHours = $attendances->sum('hours_worked');
                $adjustedTotalHours = max(0, $totalWorkedHours - 1.6); // Subtract 1.6 hours as per example
                $totalWorkedDays = $adjustedTotalHours > 0 ? round($adjustedTotalHours / 8, 2) : 0;
                ...
                $basePay = $paySchedule === 'semi-monthly' ? $dailyRate * $totalWorkedDays : ($dailyRate * $totalWorkedDays) - $basePayPenalty;
                ...
                $totalAbsences = max(0, $totalWorkingDays - ($totalWorkedDays + $leaveDays));
                ...
                if ($payroll->status !== 'Approved' || $payroll->site_id !== $employee->site_id) {
                    ...
                    $payroll->days_worked = $totalWorkedDays;
