...
{
    // Validation rules for store method (static parts only)
    private $storeValidationRules = [
        ...
        'payment_methods.*.id' => ['nullable', 'exists:employee_payment_methods,id'],
        'payment_methods.*.type' => ['required', 'string'],
        'payment_methods.*.bank_name' => ['nullable', 'string', 'max:255'],
        'payment_methods.*.bank_account_number' => ['nullable', 'string', 'max:255'],
        'payment_methods.*.account_name' => ['nullable', 'string', 'max:255'],
        'payment_methods.*.ewallet_name' => ['nullable', 'string', 'max:255'],
        'payment_methods.*.ewallet_number' => ['nullable', 'string', 'max:255'],
    ];

    // Validation rules for update method (static parts only)
    ...
    public function index()
    {
        ...
        $employees = Employee::with([
            ...
            'leave_credits', 'sss', 'philhealth', 'pagibig', 'tin', 'avatar',
        ])->get();
        ...
        return Inertia::render('Employees/EmployeeComponent', [
            ...
            'generate_superAdmin_id' => Employee::generateNextSuperAdminId(),
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $validationRules = array_merge($this->storeValidationRules, [
                ...
                'payment_methods.*.type' => ['required', Rule::in(['bank', 'ewallet'])],
                'email' => ['nullable', 'email', 'unique:users,email'],
                'password' => ['nullable', 'min:8'],
                'username' => ['nullable', 'unique:users,username'],
            ]);
            ...
            if (! empty($validated['payment_methods'])) {
                foreach ($validated['payment_methods'] as $method) {
                    Log::debug('Processing payment method', [
                        'employee_id' => $employee->id,
                        'type' => $method['type'],
                        'details' => $method,
                    ]);
                    $employee->paymentMethods()->create([
                        'type' => $method['type'],
                        'bank_name' => $method['type'] === 'bank' ? $method['bank_name'] : null,
                        'bank_account_number' => $method['type'] === 'bank' ? $method['bank_account_number'] : null,
                        'account_name' => $method['type'] === 'bank' ? $method['account_name'] : null,
                        'ewallet_name' => $method['type'] === 'ewallet' ? $method['ewallet_name'] : null,
                        'ewallet_number' => $method['type'] === 'ewallet' ? $method['ewallet_number'] : null,
                    ]);
                }
            }
    ...
    public function update(Request $request, Employee $employee)
    {
        Log::debug('Update request received', [
            ...
            'employee_employee_id' => $employee->employee_id ?? 'unknown',
            'employee_exists' => $employee->exists,
        ]);

        if (! $employee->exists) {
            Log::error('Employee not found for update', ['id' => $request->route('id')]);
        ...
        try {
            ...
            $validationRules = array_merge($this->updateValidationRules, [
                ...
                'email' => [
                    'nullable',
                    'email',
                    Rule::unique('users', 'email')->ignore($request->input('userID')),
                ],
                'password' => ['nullable', 'min:8'],
                'username' => ['nullable',
                Rule::unique('users', 'username')->ignore($request->input('userID'))],
            ]);
            ...
            if (! empty($validated['payment_methods'])) {
                $existingMethodIds = $employee->paymentMethods()->pluck('id')->toArray();
                $submittedMethodIds = array_filter(array_column($validated['payment_methods'], 'id'), fn ($id) => ! empty($id));

                // Delete removed payment methods
                $methodsToDelete = array_diff($existingMethodIds, $submittedMethodIds);
                if (! empty($methodsToDelete)) {
                    $employee->paymentMethods()->whereIn('id', $methodsToDelete)->delete();
                    Log::info('Deleted payment methods', ['employee_id' => $employee->id, 'method_ids' => $methodsToDelete]);
                }
                ...
                foreach ($validated['payment_methods'] as $method) {
                    ...
                    $employee->paymentMethods()->updateOrCreate(
                        [
                            'id' => $method['id'] ?? null,
                            'employee_id' => $employee->id,
                        ],
                ...
                // Delete all payment methods if none are submitted
                $employee->paymentMethods()->delete();
                Log::info('Deleted all payment methods', ['employee_id' => $employee->id]);
            }
        ...
        try {
            ...
            foreach ($rows as $index => $row) {
                $employeeData = [
                    ...
                    'payment_methods' => ! empty($row[27]) ? json_decode($row[27], true) : null,
                ];
