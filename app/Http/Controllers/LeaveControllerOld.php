...
class LeaveControllerOld extends Controller
{
    ...
    //     // Fetch all leaves with employee relationship
    //     $allLeaves = Leave::with('employee')->get();

    //     // My Leaves (user's own leave requests)
    //     $myLeaves = $allLeaves->where('employee_id', $user->employee->id)->values();

    //     // Log myLeaves for debugging
    //     Log::info('My Leaves: ', ['myLeaves' => $myLeaves->toArray()]);

    //     // Leave Requests (based on role)
    ...
    //                 'status' => $leave->status,
    //             ];
    //         })->all(), // Convert to array for Inertia
    ...
    public function index()
    {
        ...
        return Inertia::render($component, [
            ...
            'leave_credits' => EmployeeLeaveCredit::with('employee', 'leaveType')
    ...
    //         createNotification($admin->employee->id, "{$employeeName} submitted a leave request.", $employeeTblId);
    ...
    //         createNotification($employeeManagerID, "{$employeeName} submitted a leave request.", $employeeTblId);
    ...
    //     return redirect()->back()->with('success', 'Leave requested!');
    // }
    ...
    //                 'approved_hr_at' => now(),
    //                 'isWithPay' => $validated['is_paid'], // Add is_paid for HR/Admin/SuperAdmin
    //             ];

    //             $leaveTypeId = LeaveType::where('name', $leave->leave_type)->value('id');

    //             $leave_credits = EmployeeLeaveCredits::where('employee_id', $leave->employee_id)
    //                 ->where('leave_type_id', $leaveTypeId)
    //                 ->first();

    //             if($leave_credits->credits <= 0){
    //                 return redirect()->back()->with('error', 'You have no leave credits for this leave type.');
    //             }

    //             if ($leave_credits) {
    //                 $leave_credits->decrement('credits', $leave->day);
    //             }
    ...
    //                     ];
    //                     // $leaveCredits->decrement('credits', $leave->day);
    ...
    //                         'rejected_hr_at' => now(),
    //                     ];
    //                     $leaveCredits->increment('credits', $leave->day);
    //                 }
    //             }
    //         } else {
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            createNotification($employeeId, $statusMessage, auth()->user()->employee->id);
