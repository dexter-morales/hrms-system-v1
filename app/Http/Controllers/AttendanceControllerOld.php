...
class AttendanceControllerOld extends Controller
{
    ...
    //     $query = EmployeeAttendance::where('employee_id', $employee->id)->select([
    //         'id', 'employee_id', 'date', 'punch_in', 'punch_out', 'break_hours', 'overtime_hours', 'status', 'notes',
    //     ]);
    ...
    //     // Recent activities: Flatten punch_in and punch_out into separate events
    //     $recentActivities = EmployeeAttendance::where('employee_id', $employee->id)
    //         ->whereNotNull('punch_in')
    //         ->select(['punch_in', 'punch_out'])
    //         ->orderBy('punch_in', 'desc')
    //         ->take(10) // Take more to ensure enough events after flattening
    //         ->get()
    ...
    //                 $activities[] = [
    //                     'type' => 'in',
    //                     'timestamp' => $attendance->punch_in,
    //                 ];
    //             }
    //             if ($attendance->punch_out) {
    //                 $activities[] = [
    //                     'type' => 'out',
    //                     'timestamp' => $attendance->punch_out,
    //                 ];
    //             }
    //             return $activities;
    //         })
    ...
    public function index(Request $request)
    {
        ...
        // // Fetch attendances with a single query
        // $attendances = $query->orderBy('date', 'desc')->get();

        // // Calculate statistics with validated time differences
        // $today = Carbon::today();
        // $startOfWeek = Carbon::now()->startOfWeek();
        // $startOfMonth = Carbon::now()->startOfMonth();

        // $calculateHours = function ($attendance) {
        //     if (!$attendance->punch_in || !$attendance->punch_out) {
        //         return 0.0;
        //     }
        //     try {
        //         $in = Carbon::createFromFormat('Y-m-d H:i:s', $attendance->punch_in);
        //         $out = Carbon::createFromFormat('Y-m-d H:i:s', $attendance->punch_out);
        //         $diffHours = $out->diffInHours($in) - ($attendance->break_hours ?? 0);
        //         return max(0, $diffHours);
        //     } catch (\Exception $e) {
        //         Log::warning('Invalid punch time format', [
        ...
        // $statistics = [
        //     'today' => [
        //         'hours' => $attendances->where('date', $today->toDateString())->sum($calculateHours),
        ...
        //     ],
        //     'week' => [
        //         'hours' => $attendances->where('date', '>=', $startOfWeek->toDateString())->sum($calculateHours),
        //     ],
        //     'month' => [
        //         'hours' => $attendances->where('date', '>=', $startOfMonth->toDateString())->sum($calculateHours),
        //         'remaining' => max(0, 120 - $attendances->where('date', '>=', $startOfMonth->toDateString())->sum($calculateHours)),
